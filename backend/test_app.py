...
# CONFIG
# =========================
API_URL = "https://your-domain.com/api/voice-detection"  # <-- replace with your deployed URL
...
def test_api_key_enforcement():
    ...
    try:
        ...
        data = {}
    if response.status_code == 401 or data.get("detail", {}).get("message") == "Invalid API key or malformed request":
        print("✅ PASS")
    else:
        print("❌ FAIL")
...
def test_language_restriction():
    ...
    try:
        ...
        data = {}
    if response.status_code == 400 and data.get("detail", {}).get("message") == "Unsupported language":
        print("✅ PASS")
    else:
        print("❌ FAIL")
        print("Response:", response.text)
